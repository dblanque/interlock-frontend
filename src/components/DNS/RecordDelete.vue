...
<template>
	<v-card>
		...
		<v-card-text class="pa-0 ma-0 pb-4">
			<v-row v-if="deleteMode == 'record'" class="pa-0 ma-8 mb-2 text-subtitle-1"
				justify="center">
			...
			<v-row v-else-if="deleteMode == 'zone'" class="pa-0 ma-8 mb-2 text-subtitle-1"
				justify="center">
			...
			<span v-if="deleteMode == 'record'">
				<v-row class="pa-0 ma-0 text-subtitle-1" justify="center"
					v-for="value, attr_key in recordObject" :key="attr_key">
			...
			<span v-else-if="deleteMode == 'zone'">
				<v-form @submit="confirmZone == currentZone ? closeDialog(true) : false"
					@submit.prevent ref="deleteZoneForm">
