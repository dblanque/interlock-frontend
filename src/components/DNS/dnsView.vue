		...
		<v-data-table
			...
			<template v-slot:top>
				...
				<v-row align="center" class="px-2 mx-1 py-0 my-0">
					<v-select v-model="zoneFilter['dnsZone']" @change="getDNSData(undefined, true)"
						:items="dns.zones" class="mx-2" />
					...
					<v-btn
						...
						:disabled="loading || zoneFilter.dnsZone == 'Root DNS Servers' || zoneFilter.dnsZone == ldap.name"
						color="red">
				...
				<v-form ref="zoneCreateForm" @submit.prevent>
					<v-expand-transition>
						<v-row v-show="showZoneAdd" align="center" class="px-2 mx-6 py-0 my-0">
							...
							<v-btn class="pa-2 mx-2" color="primary" outlined @click="createZone"
								:disabled="zoneToCreate.length == 0 || loading">
						...
						<v-menu offset-y left nudge-bottom="1rem" :close-on-content-click="false"
							...
							<template v-slot:activator="{ on, attrs }">
								<v-btn v-bind="attrs" v-on="on"
									style="min-width: 32px;" class="pa-0 px-2 pr-1"
									:dark="!isThemeDark($vuetify)" :light="isThemeDark($vuetify)">
									{{ $t("actions.filter") }}
									<v-icon id="filterListButton"
										:class="filterListOpen == true ? 'active' : ''">
										mdi-chevron-down
									</v-icon>
								</v-btn>
							</template>
						...
						<template>
							<th v-for="header in props.props.headers" :key="header.value" :class="($vuetify.breakpoint.mdAndUp ? '' : 'text-center') + ' py-2 px-0'">
								<v-btn class="py-4"
									...
									<v-icon
										:class="$vuetify.breakpoint.mdAndUp ? 'mr-2' : ''">flip_to_back</v-icon>
			...
			<template v-slot:[`group.header`]="{ group, groupBy, headers, toggle, isOpen, remove }">
				<td :colspan="headers.length">
					...
					<span class="mx-5 font-weight-bold">{{ $t("dns.attributes." + groupBy) + ": " +
						group }}</span>
			...
			<template v-slot:[`item.value`]="{ item }">
				...
				<span v-else-if="item.typeName == 'SOA'">
					{{ item.namePrimaryServer + " " + item.zoneAdminEmail + " " + item.dwSerialNo +
						" " + item.dwRefresh + " " + item.dwRetry + " " + item.dwExpire + " " +
						item.dwMinimumTtl }}
				</span>
			...
			<template v-slot:[`item.ts`]="{ item }">
				...
				<v-tooltip color="primary" bottom v-if="item.ts === true">
					...
					<span>{{ $t('actions.clickTo') + ' ' + $t('actions.enable') + ' ' +
						$tc('classes.dns.record', 1)
					}}</span>
				</v-tooltip>
				...
				<v-tooltip color="red" bottom v-else>
					...
					<span>{{ $t('actions.clickTo') + " " + $t('actions.disable') + ' ' +
						$tc('classes.dns.record',
							1) }}</span>
				</v-tooltip>
			</template>
			...
			<template v-slot:expanded-item="{ headers, item }">
				<td :colspan="headers.length">
					<v-row justify="start" class="ma-0 pa-0 my-3 mx-2"
						v-for="attribute, key in getExtrasFromRecord(item, headers)" :key="key">
